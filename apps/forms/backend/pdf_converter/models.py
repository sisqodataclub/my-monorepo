...
class PDFUser(models.Model):
    """Custom user model for the PDF Converter app (decoupled from Django auth)."""
...
class EmailLoginCode(models.Model):
    """Store magic link login codes (7-day expiry) linked to PDFUser."""
...
class PDFExport(models.Model):
    """Track every PDF generation (linked to PDFUser)."""
    STATUS_CHOICES = (
        ('pending', 'Pending'),
