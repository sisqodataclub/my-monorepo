...
const Counter = ({ value, onChange }) => {
  ...
  return (
    <div className="flex items-center gap-2">
      <button onClick={minus} className="px-3 py-1 bg-gray-700 rounded text-lg font-bold text-white hover:bg-gray-600 transition">–</button>
      <span className="w-6 text-center text-lg font-semibold">{value}</span>
      <button onClick={plus} className="px-3 py-1 bg-blue-500 text-white rounded text-lg font-bold hover:bg-blue-600 transition">+</button>
    </div>
  );
};

const CleaningSelector = ({ values, setValues, carpetsServices }) => {
        ...
        {carpetsServices.map((service) => {
          ...
          return (
            ...
            >
              ...
              <Counter value={count} onChange={(v) => updateValue(service.id, v)} />
              {active && (
                <div
