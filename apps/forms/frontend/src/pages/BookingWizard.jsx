...
export default function BookingWizard() {
  ...
  const handleSubmit = async () => {
    ...
    normalAreas.forEach(area => { normalQuantities[area] = quantities[area] ?? 1; });

    const allQuantities = {
      ...sizedAreas, ...normalQuantities, ...carpets, ...appliances,
      furnished_fee: furnishedFee, biohazard_fee: details.biohazard ? biohazardFee : 0,
      discount: discount ?? 0,
    };
    ...
    try {
      const payload = {
        ...
        ...details,
        selected_areas: [service, ...normalAreas], // Kept for secure backend pricing
        quantities: allQuantities,                 // Kept for secure backend pricing
        total: finalTotal,
        payment_method: details.payment_method,
        items_breakdown: breakdown,                // ✅ Send the perfect breakdown for the email!
      };
      ...
      if (err.response && err.response.data) {
        ...
        const firstMessage = Array.isArray(backendErrors[firstKey]) ? backendErrors[firstKey][0] : backendErrors[firstKey];
